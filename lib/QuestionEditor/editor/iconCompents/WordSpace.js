...
var toggleBlock = function toggleBlock(editor, format, num, itemList) {
  // 获取之前设置的值
  var rawData = isBlockActiveNum(editor, format, itemList); // Transforms.setNodes(editor, {
  //     [format]: rawData !== num ? num : false,
  // });

  console.log(rawData, num);

  if (num === '0') {
    _slate.Editor.removeMark(editor, format);
  } else {
    _slate.Editor.addMark(editor, format, num);
  }
}; // 获取选中的数据
...
var WordSpace = function WordSpace(props) {
  ...
  function onClick(_ref) {
    ...
    var editor = props.editor,
        format = props.format,
        itemList = props.itemList; // props.onKeyDown(key);
  ...
  return /*#__PURE__*/_react.default.createElement("div", null, /*#__PURE__*/_react.default.createElement(_components.Button, {
    ...
    active: props.active !== props.itemList[0],
